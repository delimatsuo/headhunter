...
CREATE TABLE candidate_embeddings (
    ...
    embedding vector(768),  -- Dimension matches embedding model (384 for sentence-transformers, 768 for others)
    model_version VARCHAR(50) NOT NULL DEFAULT 'vertex-ai-textembedding-gecko',
    chunk_type VARCHAR(50) DEFAULT 'full_profile',  -- 'full_profile', 'skills', 'experience', etc.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    metadata JSONB,
    
    -- Ensure idempotent upserts as per PRD line 143
...
-- HNSW index for exact nearest neighbor search (slower, more accurate)
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON candidate_embeddings TO your_app_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON embedding_metadata TO your_app_user;
