#!/usr/bin/env python3
"""
Re-embed all enriched candidates from PostgreSQL with structured profile data.
"""
...
def build_searchable_profile(candidate: Dict[str, Any]) -> str:
    ...
    def get_field(data: Dict, path: str) -> Optional[Any]:
    ...
    def get_array(data: Dict, path: str) -> List[str]:
    ...
    def get_string(data: Dict, path: str) -> Optional[str]:
        """Get string from path"""
        value = get_field(data, path)
        return str(value) if isinstance(value, str) else None

    def get_number(data: Dict, path: str) -> Optional[int]:
        """Get number from path"""
        value = get_field(data, path)
        return int(value) if isinstance(value, (int, float)) else None

    # Get enriched_data JSON
    enriched_data = candidate.get('enriched_data', {})
    if not enriched_data:
        enriched_data = {}

    # 1. Technical Skills (HIGHEST PRIORITY)
    primary_skills = get_array(enriched_data, 'technical_assessment.primary_skills')
    core_competencies = get_array(enriched_data, 'skill_assessment.technical_skills.core_competencies')
    ...
    current_role = get_string(enriched_data, 'experience_analysis.current_role')
    ...
    total_years = get_number(enriched_data, 'experience_analysis.total_years')
    years_experience = get_number(enriched_data, 'career_trajectory.years_experience')
    years = total_years if total_years is not None else years_experience
    if years is not None:
        parts.append(f"Experience: {years} years")

    seniority_level = get_string(enriched_data, 'personal_details.seniority_level')
    current_level = get_string(enriched_data, 'career_trajectory.current_level')
    seniority = seniority_level or current_level
    if seniority:
        parts.append(f"Seniority: {seniority}")

    # 4. Domain Expertise
    domain_expertise = get_array(enriched_data, 'skill_assessment.domain_expertise')
    if domain_expertise:
        parts.append(f"Domain: {', '.join(domain_expertise[:5])}")

    # 5. Role Type (IC vs Leadership)
    has_leadership = get_field(enriched_data, 'leadership_scope.has_leadership')
    team_size = get_number(enriched_data, 'leadership_scope.team_size')
    ...
    # 6. Ideal Roles
    ideal_roles = get_array(enriched_data, 'recruiter_recommendations.ideal_roles')
    best_fit_roles = get_array(enriched_data, 'recruiter_insights.best_fit_roles')
    roles = list(set(ideal_roles + best_fit_roles))
    if roles:
        parts.append(f"Best Fit: {', '.join(roles[:5])}")

    # 7. Executive Summary
    one_liner = get_string(enriched_data, 'executive_summary.one_line_pitch')
    if one_liner:
        parts.append(f"Summary: {one_liner}")

    # 8. Searchability Keywords
    keywords = get_array(enriched_data, 'searchability.keywords')
    search_tags = get_array(enriched_data, 'search_optimization.keywords')
    all_keywords = list(set(keywords + search_tags))
    if all_keywords:
        parts.append(f"Keywords: {', '.join(all_keywords[:20])}")

    # 9. Company Pedigree
    company_tier = get_string(enriched_data, 'company_pedigree.company_tier')
    if company_tier:
    ...
    if not parts:
        ...
        if resume_text:
            print(f"WARNING: No enriched data found, falling back to resume_text")
            return resume_text
...
async def reembed_candidate(
    ...
    try:
        profile_text = build_searchable_profile(candidate)
        ...
        async with session.post(f"{embed_url}/v1/embeddings/upsert", json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            if resp.status >= 200 and resp.status < 300:
                name = candidate.get('full_name', 'Unknown')
                title = candidate.get('current_title', 'Unknown')
                print(f"✓ {candidate_id} ({name} - {title}): Re-embedded successfully")
                return True
            else:
                error_text = await resp.text()
                print(f"✗ {candidate_id}: Failed with {resp.status}: {error_text[:200]}")
                return False
...
async def main():
    ...
    db_host = os.getenv("DB_HOST", "/cloudsql/headhunter-ai-0088:us-central1:sql-hh-core")
    ...
    query = f"""
        SELECT id, full_name, current_title, enriched_data, resume_text
    ...
    async with aiohttp.ClientSession() as session:
        for i in range(0, len(candidates), batch_size):
            ...
            tasks = [
                reembed_candidate(session, embed_url, tenant_id, candidate, api_key)
