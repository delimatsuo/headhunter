...
must_haves:
  truths:
    - "Query expansion uses skills-graph.ts copied to services workspace"
  ...
  artifacts:
    - path: "services/hh-search-svc/src/shared/skills-graph.ts"
      provides: "Skills ontology graph for query expansion"
      exports: ["getCachedSkillExpansion", "expandSkills"]
    - path: "services/hh-search-svc/src/shared/skills-master.ts"
      provides: "Master skills database"
      exports: ["MASTER_SKILLS", "getSkillByName"]
  ...
  key_links:
    - from: "services/hh-search-svc/src/nlp/query-expander.ts"
      to: "services/hh-search-svc/src/shared/skills-graph.ts"
      via: "import getCachedSkillExpansion"
      pattern: "from.*shared/skills-graph"
---
...
# Source files to copy from functions workspace
...
<task type="auto">
  <name>Task 0: Copy skills ontology to services workspace</name>
  <files>
    services/hh-search-svc/src/shared/skills-master.ts
    services/hh-search-svc/src/shared/skills-graph.ts
  </files>
  <action>
Copy the skills ontology files from functions workspace to services workspace to enable direct imports without cross-workspace path issues.

1. Create the shared directory:
```bash
mkdir -p services/hh-search-svc/src/shared
```

2. Copy skills-master.ts from functions/src/shared/skills-master.ts to services/hh-search-svc/src/shared/skills-master.ts

The file contains:
- MASTER_SKILLS array with 200+ skills
- Skill type definition
- Helper functions: getSkillByName, getSkillById, matchSkillName, etc.

3. Copy skills-graph.ts from functions/src/shared/skills-graph.ts to services/hh-search-svc/src/shared/skills-graph.ts

Update the import path in skills-graph.ts from:
```typescript
import { MASTER_SKILLS, getSkillByName, type Skill } from './skills-master';
```
To (same path, since it's in the same directory):
```typescript
import { MASTER_SKILLS, getSkillByName, type Skill } from './skills-master';
```
(No change needed - relative import stays the same)

4. Suppress the console.log statements in skills-graph.ts for production use:
- Change `console.log` to use a conditional that checks for NODE_ENV !== 'test' or remove them
- Or wrap them in a DEBUG check

The key exports needed:
- `getCachedSkillExpansion(skillName: string, maxDepth?: number): SkillExpansionResult`
- `expandSkills(skillName: string, maxDepth?: number, maxResults?: number): SkillExpansionResult`
- `SkillExpansionResult` type
  </action>
  <verify>
Files exist and compile:
```bash
ls services/hh-search-svc/src/shared/skills-master.ts
ls services/hh-search-svc/src/shared/skills-graph.ts
cd services/hh-search-svc && npm run typecheck
```
  </verify>
  <done>
Skills ontology files copied to services workspace, imports work within same workspace.
  </done>
</task>

<task type="auto">
  <name>Task 1: Implement QueryExpander using local skills-graph</name>
  <files>
    services/hh-search-svc/src/nlp/query-expander.ts
  </files>
  <action>
Create query-expander.ts that wraps the local skills ontology:
...
- Import path changed from `../../../../functions/src/shared/skills-graph` to `../shared/skills-graph`
- This is a deterministic import that will work because Task 0 copies the files
  </action>
  <verify>
TypeScript compiles:
```bash
...
```
  ...
  <done>
QueryExpander class exists using local skills-graph.ts with configurable depth, confidence threshold, and expansion limits.
  </done>
</task>
...
- Mock path changed from `../../../../../functions/src/shared/skills-graph` to `../../shared/skills-graph`
  </action>
  <verify>
Run the tests:
```bash
...
<verification>
1. Skills ontology files copied: `ls services/hh-search-svc/src/shared/skills-*.ts`
2. TypeScript compiles: `cd services/hh-search-svc && npm run typecheck`
3. Tests pass: `npm test -- src/nlp/__tests__/query-expander.spec.ts`
4. QueryExpander:
...
<success_criteria>
- Skills ontology copied to services/hh-search-svc/src/shared/
- QueryExpander class using local skills-graph.ts (no cross-workspace imports)
