...
<task type="auto">
  <name>Task 2: Replace skill display with SkillChip components</name>
  <files>headhunter-ui/src/components/Candidate/SkillAwareCandidateCard.tsx</files>
  <action>
Update the skills cloud section (around lines 546-562) to use SkillChip:

1. Collect both explicit and inferred skills from candidate data:
```typescript
// Inside component, derive skill display data
const getSkillsForDisplay = () => {
  ...
  // Map to SkillChip format
  const explicitChips = explicit.map(s => ({
  ...
  const inferredChips = [
    ...inferredHigh.map(s => ({ skill: s.skill, type: 'inferred' as const, confidence: s.confidence, evidence: s.reasoning })),
    ...inferredMedium.map(s => ({ skill: s.skill, type: 'inferred' as const, confidence: s.confidence, evidence: s.reasoning })),
    ...inferredLow.map(s => ({ skill: s.skill, type: 'inferred' as const, confidence: s.confidence, evidence: s.reasoning })),
  ];

  return [...explicitChips, ...inferredChips].slice(0, 15); // Limit to 15 total
};
```

2. Update skill cloud rendering to use SkillChip:
```tsx
...
3. Remove or deprecate old skill rendering code that doesn't show confidence
  </action>
  <verify>
...
- Hover shows evidence tooltip for inferred skills
  ...
  <done>Skills cloud uses SkillChip with confidence indicators</done>
</task>
