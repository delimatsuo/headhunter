...
**IMPORTANT - Type access pattern clarification:**
Research confirms signalScores is at the RESULT level, not nested under candidate:
- Correct: `match.signalScores?.skillsExactMatch`
- WRONG: `match.candidate?.signalScores?.skillsExactMatch`

Ensure CandidateMatch interface (from 09-01) includes signalScores directly.
...
const getSortedAndFilteredMatches = () => {
  ...
  // NOTE: signalScores is at result level per HybridSearchResultItem
  let filtered = matches.filter(m => {
    const skillScore = m.signalScores?.skillsExactMatch ?? 0.5; // neutral if missing
    return skillScore * 100 >= minSkillScore;
  });

  // Sort based on selection
  return filtered.sort((a, b) => {
    // Type-safe helper - signalScores is on CandidateMatch, not nested
    const getSignalScore = (match: CandidateMatch, signal: keyof SignalScores): number => {
      return match.signalScores?.[signal] ?? 0.5;
    };
