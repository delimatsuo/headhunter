...
<task type="auto">
  ...
  <action>
At line 146 where vectorPool is initialized from vectorSearchResults, map the results to preserve the raw similarity score:
...
This ensures the raw vector similarity from vector-search.ts (which sets `vector_similarity_score` at line 839) is preserved under a new private field `_raw_vector_similarity` that won't be confused with other score fields.
