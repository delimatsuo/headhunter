...
<task type="auto">
  <name>Task 2: Deploy and start the development server</name>
  <files>N/A (deployment)</files>
  <action>
Build and start both the backend functions and frontend for testing:

1. Deploy Firebase functions (or run emulator):
   ```bash
...
3. If using production, verify Cloud Run services are deployed with the updated code:
   ```bash
   gcloud run services describe hh-search-svc --region=us-central1 --format="value(status.latestReadyRevisionName)"
   ```

Note: The exact deployment method depends on your environment setup. The goal is to have both backend and frontend running with the changes from Plans 01-03.
  </action>
  <verify>
Frontend accessible at http://localhost:3000 (or production URL)
Backend responding to search requests (check network tab)
  </verify>
  <done>
