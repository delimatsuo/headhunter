...
export class EcoRedisClient {
  ...
  private ensureClient(): Redis | Cluster | null {
    ...
    if (hosts.length > 1) {
      ...
      this.logger.info({ tls: tlsOptions ?? null, cluster: true }, 'Initializing ECO Redis cluster client.');
      ...
      this.logger.info({ tls: tlsOptions ?? null, cluster: false, host: options.host, port: options.port }, 'Initializing ECO Redis client.');
