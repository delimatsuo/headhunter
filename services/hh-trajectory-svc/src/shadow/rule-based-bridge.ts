/**
 * Rule-Based Trajectory Bridge for Shadow Mode Comparison
 *
 * DUPLICATED FROM hh-search-svc/src/trajectory-calculators.ts - Keep in sync
 *
 * This module implements Phase 8 rule-based trajectory calculation logic for
 * shadow mode comparison with ML predictions. Duplicated to avoid inter-service
 * HTTP calls during shadow logging.
 */
...
/**
 * Rule-based trajectory metrics for shadow comparison.
 */
 ...
 * Simplified version without Together AI fallback for shadow mode.
 *
 * Rules:
...
// ============================================================================
// Trajectory Fit Scoring (Simplified for Shadow Mode)
// ============================================================================
...
/**
 * Calculates simplified trajectory fit score (0-1).
 * Simplified version that only uses direction + velocity.
 *
 * @param direction - Trajectory direction
 * @param velocity - Trajectory velocity
...
export function calculateTrajectoryFit(
  ...
  return DIRECTION_VELOCITY_SCORES[direction]?.[velocity] ?? 0.5;
}

// ============================================================================
// Rule-Based Bridge Class
// ============================================================================

/**
 * Bridge class that provides rule-based trajectory computation for shadow mode comparison.
 */
export default class RuleBasedBridge {
  /**
   * Computes rule-based trajectory metrics from title sequence and experiences.
   *
   * @param titleSequence - Array of job titles in chronological order
   * @param experiences - Optional experience entries with dates
   * @returns Complete rule-based trajectory metrics
   */
  compute(titleSequence: string[], experiences?: ExperienceEntry[]): RuleBasedMetrics {
    const direction = calculateTrajectoryDirection(titleSequence);
    const velocity = calculateTrajectoryVelocity(experiences || []);
    const type = classifyTrajectoryType(titleSequence);
    const fit = calculateTrajectoryFit(direction, velocity);
